...
function CartPage() {
 ...
 const fetchCart = async () => {
  ...
  try {
    let res;
    if (token) {
      res = await axios.get(`${API_BASE_URL}/api/cart`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    } else {
      res = await axios.get(`${API_BASE_URL}/api/cart/session/${sessionId}`, {
  ...
  return (
    ...
    <div className="hidden md:block md:col-span-1" />
        ...
        Bag {totalItems > 0 && <span className="text-zinc-900">({totalItems} item{totalItems > 1 ? 's' : ''})</span>}
      </h1>
              ...
              {cartItems.map((item) => (
                <div key={item.product_id + item.size} className="flex gap-4 border-b pb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {item.quantity === 1 ? (
                        <svg aria-hidden="true" focusable="false" viewBox="0 0 24 24" role="img" width="24px" height="24px" fill="none"><path stroke="currentColor" stroke-miterlimit="10" stroke-width="1.5" d="M13.75 10v7m-3.5-7v7m-3.5-8.5V17c0 1.24 1.01 2.25 2.25 2.25h6c1.24 0 2.25-1.01 2.25-2.25V7.75h2.25m-10-3h3.75c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5H4.5"></path></svg>
                      ) : (
                        <svg aria-hidden="true" focusable="false" viewBox="0 0 24 24" role="img" width="24px" height="24px" fill="none"><path stroke="currentColor" stroke-miterlimit="10" stroke-width="1.5" d="M18 12H6"></path></svg>
                      )}
                    </button>
                    ...
                    <button
                      ...
                      onClick={() => updateQuantity(item.product_id, item.quantity + 1, item.size)}
                    >
                      <svg aria-hidden="true" focusable="false" viewBox="0 0 24 24" role="img" width="24px" height="24px" fill="none"><path stroke="currentColor" stroke-miterlimit="10" stroke-width="1.5" d="M18 12H6m6 6V6"></path></svg>
                    </button>
                  </div>
                </div>
              ))}
